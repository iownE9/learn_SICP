def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return sum([[[item] + i] for i in nested_list], [])
...
def inc_subseqs(s):
    ...
    #         return insert_into_all(prev, a) + b
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    """
    # 4 叶子的画出来， 一层一层对比 递归调用的函数传参 就会发现规律
    # BearSir's
    if n < 3:
        return 1
...
class Keyboard:
    ...
    # positions as keys, and values as Buttons
...
def make_advanced_counter_maker():
    ...
    def make_counter(mine_count=0):
        # 加不加都行
        nonlocal global_count

        def counter(str):
            nonlocal global_count, mine_count
            "*** YOUR CODE HERE ***"
...
def insert(link, value, index):
    ...
    if index == 0 and link is not Link.empty:
        ...
        # print('IndexError')  # mine

        raise IndexError  # BearSir
