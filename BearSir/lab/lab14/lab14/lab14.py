...
def prune_min(t):
    ...
    if t.is_leaf():
        return
    min_label = min([b.label for b in t.branches])
    for b in t.branches:
        if min_label == b.label:
            t.branches = [b]
            prune_min(b)
...
def num_splits(s, d):
    ...
    def helper(s):
        if len(s) <= 1:
            return s
        return sum([[s[0] + i, s[0] - i] for i in helper(s[1:])], [])

    return len([i for i in helper(s) if abs(i) <= d])

    # BearSir's  树型递归遍历经典写法
    def split(s, n):
        if len(s) == 0:
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.payable_to == self.holder and not check.deposited:
            ...
            return self.deposit(check.dollar)
        else:
