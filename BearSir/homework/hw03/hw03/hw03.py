...
def totals_tree(m):
    ...
    return tree(
        total_weight(m),
        [totals_tree(left_end), totals_tree(right_end)])
...
def replace_leaf(t, find_value, replace_value):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        # mine  [replace_value] 打破了抽象壁垒
        # return [replace_value] if label(t) == find_value else t

        # Bearsir's
        return tree(replace_value) if label(t) == find_value else tree(
            label(t))
...
def quadratic(x, a, b, c):
    ...
    left, mid, right = lower_bound(x), -b / (2 * a), upper_bound(x)
