class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            return 'You must add ${0} more funds.'.format(self.price -
                                                          self.fund)
        ...
        if self.fund == self.price:
            ...
            self.fund, blance = 0, self.fund - self.price
            return 'Here is your {0} and ${1} change.'.format(
                self.name, blance)
...
def is_bst(t):
    ...
    # bst_min, bst_max 这类工具函数功能要 单一明确
    def bst_min(t):  # 最小值
    ...
    def res(t):
        ...
        if length == 2:
            b0, b1 = t.branches[0], t.branches[1]
            b_bool = bst_max(b0) <= t.label <= bst_max(b1)

            return res(b0) and res(b1) if b_bool else False

        elif length == 1:
            b = t.branches[0]
            b_bool = bst_max(b) <= t.label or t.label <= bst_max(b)
            return res(b) if b_bool else False
