...
(define (enumerate s)
  ...
  ; (map (lambda (e) (list (length s) e)) s)
 
  ; BearSir's
  (define (enum-helper lst idx)
    (if (null? lst) nil
      (cons (list idx (car lst)) (enum-helper (cdr lst) (+ idx 1)))
    )
  )
  (enum-helper s 0)
    ...
    (cond 
        ...
        ((<= (car s) (cadr s)) 
            (let ((rest (nondecreaselist (cdr s))))
                ...
                (if (null? (cdr rest))
                    (list (append (list (car s)) (car rest)))  ; ((4)) cdr -> ()
                    (append (list (append (list (car s)) (car rest)))
                            (cdr rest) ; ((4) (3)) cdr -> ((3))
                    )  
                ) 
            )
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           ; BEGIN PROBLEM EC
           ;(append (list(list 'lambda (car (zip values)) (car body)))
           ;        (cadr (zip values)))
           ; BearSir
           (cons
