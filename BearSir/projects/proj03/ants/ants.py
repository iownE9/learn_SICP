...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.is_container():
                container_ant, ant_ = self, place.ant
            else:
                container_ant, ant_ = place.ant, self

            if container_ant.can_contain(ant_):
    ...
    def buff(self):
        ...
        if not (hasattr(self, 'buff_') and self.buff_):
            self.damage *= 2
            self.buff_ = True
            # print("DEBUG:", self)
...
def bee_selector(bees):
    ...
    assert isinstance(
        bees,
        list), "bee_selector's argument should be a list but was a %s" % type(
            bees).__name__
    if bees:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, health=1):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if QueenAnt.first_QueenAnt:
            self.is_QueenAnt = False
        else:
            QueenAnt.first_QueenAnt, self.is_QueenAnt = True, True
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_health(amount)
        if self.is_QueenAnt and self.health == 0:
            bees_win()
